...
function ChallengeCard() {
  const quiz = useSelector((state) => state.takeChallenge.quiz);
    ...
    <Container>
      <Card>
        <Card.Header>
          ...
          <h4>
            Question: {quizNo + 1} of {questionNumber}
          </h4>
          ...
          <Row>
            <Col lg={8} sm={12}>
              <Card.Text className="mb-4">
                <h4> {quiz[quizNo].subject.replaceAll('"', "")} </h4>
            ...
            <Col lg={3}>
              {quiz[quizNo].options.map((option, index) => {
                return (
                  <FormGroup
                    check
                    className="take-quiz-options mb-1"
                    key={index}
                  >
                    <Input
                      name="options"
                      ...
                      value={index + 1}
                      id={option}
                      onChange={(event) => saveSelected(event)}
                    />{" "}
                    <Label
                      check
                      for={option}
